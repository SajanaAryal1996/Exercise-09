...
**The results obtained from both the `lm()` function and manual calculations yield the same result.**
...
# Create a new dataframe by removing missing variables
s <- d %>% drop_na(ECV, Group_size) %>%
  mutate(logECV = log(ECV), logGS = log(Group_size))
...
# Manually calculate standard error, 95% CI, and p-value
n <- nrow(s)  #
...
# Extract coefficient standard error, 95% CI, and p-value from lm() results
...
for (i in 1:n) {
  ...
  permuted_response <- sample(transformed_data$logECV)
  
  # Fit a linear model with permuted data
  permuted_model <- lm(permuted_response ~ transformed_data$logGS)
